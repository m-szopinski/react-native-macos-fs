...
export const readDir = async (path) => {
    try {
        ...
        return items.map((item) => (Object.assign(Object.assign({}, item), { isFile: () => item.type === 'file', isDirectory: () => item.type === 'directory' })));
    }
    catch (err) {
...
export const stat = async (path) => {
    try {
        ...
        return Object.assign(Object.assign({}, result), { isFile: () => result.type === 'file', isDirectory: () => result.type === 'directory' });
    }
    catch (err) {
